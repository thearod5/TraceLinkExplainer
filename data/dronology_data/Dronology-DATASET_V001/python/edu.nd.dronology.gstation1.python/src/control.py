...
from common import *
from communication.core import RTKConnection
...
class ControlStation(object):
    ...
    """
    def __init__(self, connection, g_id,
                 dronology_in_msg_queue,
                 dronology_handshake_out_msg_queue,
                 dronology_state_out_msg_queue,
                 new_vehicle_queue,
                 rtk_config=None):
        ...
        self._workers = [self._v_in_worker, self._d_in_worker, self._d_out_worker]

        self._rtk_connection = None
        self._rtk_msgs = None
        self._rtk_drone_ids = []

        if rtk_config is not None:
            self._rtk_msgs = []
            addr = rtk_config['address']
            port = rtk_config['port']
            self._rtk_connection = RTKConnection(self._rtk_msgs, addr, port)
            self._rtk_worker = threading.Thread(target=self._rtk_work)
            self._workers.append(self._rtk_worker)
    ...
    def _rtk_work(self):
        """
        This is the rtk worker thread.

        It is responsible for forwarding messages received from the RTK connection to all drones that are equipped with
        RTK.
        """
        cont = True
        while cont:
            if self._rtk_msgs:
                msg = self._rtk_msgs.pop(0)
                for drone_id in self._rtk_drone_ids:
                    self._drones[drone_id].send_rtk_adjustment(msg)
    ...
    def _register_vehicle(self, v_spec):
        ...
        vehicle = ArduCopter(self._d_handshake_out_msgs, self._d_state_out_msgs, self._g_id)

        vehicle_id = v_spec['vehicle_id']
        rtk_enabled = v_spec.get('rtk_enabled', False)
        with self._drone_lock:
            ...
            # if RTK is enabled, add this drone to the list of rtk enabled drones
            if rtk_enabled:
                self._rtk_drone_ids.append(vehicle_id)
