...
class VehicleControl(object):
    def __init__(self, handshake_msg_queue, state_msg_queue, groundstation_id,
                 vehicle_id=None, state_interval=1.0, monitor_interval=1.0, gps_rtcm_sequence_num=0):
        self._vehicle = None
        ...
        self._monitor_t = monitor_interval
        self._state_msg_timer = util.etc.RepeatedTimer(self._state_t, self.send_state_message)
        self._monitor_msg_timer = util.etc.RepeatedTimer(self._monitor_t, self.send_monitor_message)
        ...
        self._gid = groundstation_id
        self._gps_rtcm_sequence_num = gps_rtcm_sequence_num
        self._timers = [self._state_msg_timer, self._monitor_msg_timer]
    ...
    def update_monitor_interval(self, monitor_interval):
        with self._msg_lock:
            if self._monitor_msg_timer is not None:
                self._monitor_msg_timer.stop()
                self._monitor_msg_timer.set_interval(monitor_interval)
                self._monitor_msg_timer.start()

    def send_monitor_message(self):
        if self._connection_successful:
            with self._msg_lock:
                msg = self.gen_monitor_message()
                self._state_out_msgs.put_message(msg)
