...
from explanation.loader.DataLoader import (get_all_artifacts_for_dataset,
                                           get_traced_artifacts)
from explanation.relationships.vsm.CalculateSimilarityMatrix import calculate_similarity_matrix
...
def create_sorted_response(artifacts, similarities, limit: int):
    ...
    for item_index in similarities.sort_values(ascending=False).index:
        search_items.append(artifacts[item_index])
    return create_search_response(search_items, limit)
